...
public class ParserCertidaoTRF1v2 implements IParserCertidao, IParserConfigurable<ParserConfigTRF1> {
	...
	public Certidao parse(Documento doc) {
		...
		pattern.searchFirst("CERTIDÃO JUDICIAL (\\S*) \\d+\\s*/\\s*\\d+");
		pattern.search("O (.*?), na forma da lei, CERTIFICA que, consultando os sistemas processuais");
		pattern.search("N[AÃ]O CONSTA");
		pattern.search("nenhuma AÇÃO de natureza .*? em andamento.*{1,100}contra\\s+(.*?)");
		pattern.search("CPF/CNPJ:\\s*(" + ParserUtil.REGEX_CPF_OU_CNPJ + ")\\.");
		
		pattern.searchNext("Certid[aã]o:\\s*(\\d+)\\s");
		pattern.searchNext("Codigo de Seguranca:\\s*([0-9A-F]+)\\s");
		pattern.searchNext("Data da Atualiza[cç][aã]o: (\\S+)\\s*às\\s*(\\S+\\s+[AP]M)");
